...
fn get_configuration_section_heading_node<'a>(root: &'a AstNode<'a>) -> Option<&'a AstNode<'a>> {
    root.children().find(|node| {
        ...
        matches!(node_data.value, NodeValue::Heading(NodeHeading { level, .. })
            if level == 2 && std::str::from_utf8(&node_data.content).unwrap_or("").trim() == CONFIGURATION_HEADING_TEXT)
    })
}
...
fn is_configuration_tag_heading_node<'a>(node: &'a AstNode<'a>) -> bool {
    let node_data = node.data.borrow();
    matches!(node_data.value, NodeValue::Heading(NodeHeading { level, .. })
            if level == 3 && std::str::from_utf8(&node_data.content).unwrap_or("").starts_with(TAG_PREFIX))
}
...
fn extract_configuration_tag_from_heading_node<'a>(node: &'a AstNode<'a>) -> String {
    ...
    heading[tag_start_index + TAG_PREFIX.len()..].to_owned()
}
...
mod tests {
    ...
    #[test]
    fn should_parse_one_configuration() {
        let input = "
## Configuration
...
";
    ...
    #[test]
    fn should_parse_multiple_configuration() {
        let input = "
## Configuration
...
";
    ...
    #[test]
    #[should_panic]
    fn should_fail_for_invalid_heading() {
        let invalid_input = "
## INVALID_HEADING
